...
pub fn parse_glyph_offsets(file: &mut FontFile, font_data: &FontData) -> io::Result<Vec<u32>> {
    file.goto(font_data.get_table(TableTag::Maxp)?.offset + 4)?; // Skip unused version
    let num_glyphs = file.read_u16()?;

    file.goto(font_data.get_table(TableTag::Head)?.offset)?;
    ...
    let location_table_start = font_data.get_table(TableTag::Loca)?.offset;
    let glyph_table_start = font_data.get_table(TableTag::Glyf)?.offset;
